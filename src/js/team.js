...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		// main container
		containerEl = document.querySelector('.team-container'),
		// close search ctrl
	...
	function initEvents() {
		...
		pins.forEach(function(pin) {
			...
			pin.addEventListener('click', function(ev) {
				if (pin.getAttribute('data-space') == null) {
					return;
				}
				ev.preventDefault();
	...
	function showLevel(level) {
		...
		console.log("showing level", level, selectedLevel);

		// We need a transition if we are moving to a new level or opening a new level
		var transitionNeeded = (!isExpanded) || (selectedLevel != level);
		...
		if (selectedLevel) {
			...
			if (transitionNeeded) {
				...
				removePins(mallLevels[selectedLevel - 1]);
			}
		}
	...
	function openContent(spacerefval) {
		...
		// Gets the mini circle step so we can animate it

		// remove class selected (if any) from current space
		removeStepAnimations();
		var stepval = spacerefval.slice(-1);
		var stepCircleEl = (mallLevels[selectedLevel - 1].querySelector('svg > .step-pulses').children)[stepval-1]; 
		var stepNumberEl = (mallLevels[selectedLevel - 1].querySelector('svg > .circle-numbers').children)[stepval-1]; 
		classie.add(stepNumberEl, 'circle-number--active');
		classie.add(stepCircleEl, 'step-pulse--active');
	...
	function removeStepAnimations() {
		var activeCircleEl = mallLevels[selectedLevel - 1].querySelector('svg .step-pulse--active');
		var activeNumberEl = mallLevels[selectedLevel - 1].querySelector('svg .circle-number--active');
		console.log("Active cirlce el", activeNumberEl);
		if( activeCircleEl ) {
			classie.remove(activeCircleEl, 'step-pulse--active');
			classie.remove(activeNumberEl, 'circle-number--active');
		}
	}
